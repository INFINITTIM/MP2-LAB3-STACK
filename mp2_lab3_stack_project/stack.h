...
public:
	...
	T Pop(); // взятие элемента с вершины стека
...
{
	if (this == &other_stack) //проверяем что эти два стека не являются одним и тем же стеком
		return *this;
	if (maxsize != other_stack.maxsize) // если размеры стэков различны то берем и удаляем память нашего стека,
	{                                  // берём размер другого стэка и приравниваем его к нашего стеку, а потом 
		delete[] pMem;                 // уже выделяем память для стека нужного размера
		maxsize = other_stack.maxsize;
		pMem = new T[maxsize];
	}
	num_last_element = other_stack.num_last_element;
	for (int i = 0; i <= num_last_element; i++) // записываем значения другого стека в свой стек 
	{
		pMem[i] = other_stack.pMem[i];
	}
}
